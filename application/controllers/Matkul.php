...
class Matkul extends CI_Controller
{
	...
	public function index()
	{
		$data = [
			...
			'judul'	=> text_fill('pelajaran'),
			'subjudul' => 'Data '.text_fill('pelajaran'),
		];
	...
	public function add()
	{
		$data = [
			'user' 		=> $this->ion_auth->user()->row(),
			'judul'		=> 'Tambah '.text_fill('pelajaran'),
			'subjudul'	=> 'Tambah Data '.text_fill('pelajaran'),
	...
	public function edit()
	{
		...
		if (!$chk) {
			...
			$data = [
				'user' 		=> $this->ion_auth->user()->row(),
				'judul'		=> 'Edit '.text_fill('pelajaran'),
				'subjudul'	=> 'Edit Data ' . text_fill('pelajaran'),
				'matkul'	=> $matkul
			];
			$this->load->view('_templates/dashboard/_header.php', $data);
	...
	public function save()
	{
		...
		for ($i = 1; $i <= $rows; $i++) 
		{
			...
			$this->form_validation->set_rules($nama_matkul, text_fill('pelajaran'), 'required');
	...
	public function import($import_data = null)
	{
		$data = [
			'user' => $this->ion_auth->user()->row(),
			'judul'	=> text_fill('pelajaran'),
			'subjudul' => 'Import '. text_fill('pelajaran')
		];
